...
Working in Epidiverse pipeline we see an usual output from the Oly data, as particular as it relates to our Geoduck WGBS..
...
```
to determine that 216 of the CT SNPs in that give vcf were in fact in CG loci. (24%)

My hope is this is some level of statistic that we can use to quantify the proportion instances where CT SNPs might influence our interpretation..

This couple with an explanation that it is ok to include the data as as CT SNP is in fact a situation where there is no methylation (similar to what BS does.)
